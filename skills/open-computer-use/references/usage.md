...
overseer computer-use call set_value --args '{"app":"TextEdit","element_index":"1","value":"Draft"}'
```
...
```sh
overseer computer-use call click --args '{"app":"Google Chrome","x":875,"y":375,"click_method":"app_post"}'
```

Use `sky_click` when Chromium ignores `app_post` and the current target window is covered by another window:

```sh
overseer computer-use call get_app_state --args '{"app":"Google Chrome"}'
overseer computer-use call click --args '{"app":"Google Chrome","x":875,"y":375,"click_method":"sky_click"}'
```
...
```sh
OPEN_COMPUTER_USE_ALLOW_GLOBAL_POINTER_FALLBACKS=1 overseer computer-use call click --args '{"app":"Google Chrome","x":875,"y":375,"click_method":"global"}'
```
